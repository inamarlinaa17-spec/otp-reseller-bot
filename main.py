...
from rumahotp import (
    ...
    complete_number as complete_rumahotp_number,
...
def format_datetime_wib(value):
    """Format stored/provider timestamps consistently for users in WIB."""
    if value in (None, "", "-"):
        return "-"
    try:
        raw = str(value).strip()
        if raw.isdigit():
            timestamp = float(raw)
            # RumahOTP returns Unix timestamps in milliseconds.
        ...
        return dt.astimezone(pytz.timezone("Asia/Jakarta")).strftime("%d/%m/%Y %H:%M:%S WIB")
    except Exception:
        return str(value)
...
async def user_start(
    ...
    maintenance_notice = "\n🛠 <b>MAINTENANCE AKTIF</b> — pemesanan OTP sementara ditutup.\n" if is_maintenance_enabled() else ""
...
def _manual_admin_url(deposit_id, amount, payment_amount):
    message = (
        ...
        f"Nominal saldo: Rp{amount:,}\n"
        f"Nominal transfer: Rp{payment_amount:,}"
    )
    if ADMIN_USERNAME:
        return f"https://t.me/{ADMIN_USERNAME}?text={quote(message)}"
...
async def _send_manual_qris_invoice_to_chat(context, chat_id, user, amount):
    ...
    await context.bot.send_photo(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Saya Sudah Bayar", callback_data=f"manual_paid:{deposit_id}")],
...
def _complete_manual_deposit(deposit_id):
    with get_db() as db:
        ...
        if deposit["payment_method"] != "MANUAL_QRIS":
            raise ValueError("Deposit ini bukan QRIS manual.")
        if deposit["status"] == "SUCCESS":
            current = db.execute("SELECT balance FROM users WHERE telegram_id=%s", (deposit["telegram_id"],)).fetchone()
            return {"completed": False, "already_completed": True, **dict(deposit), "new_balance": int((current or {}).get("balance") or 0)}
        if deposit["status"] != "PENDING":
            raise ValueError(f"Deposit berstatus {deposit['status']}.")
        ...
        bonus = int(amount * 0.10) if amount >= 100000 else 0
        ...
        db.execute("INSERT INTO ledger (telegram_id,amount,balance_before,balance_after,transaction_type,reference,description,created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", (deposit["telegram_id"], amount, before, before + amount, "DEPOSIT", deposit_id, f"Deposit QRIS Manual {deposit_id}", now()))
        if bonus:
...
async def command_deposit(update, context):
    ...
    await update.message.reply_text(
        ...
        "⚡ <b>Pembayaran Otomatis</b> — pembayaran otomatis dan saldo diproses setelah terverifikasi.\n"
        "📷 <b>QRIS Manual</b> — scan QRIS admin lalu konfirmasi pembayaran.\n\n"
...
# =========================================================
# AUTOMATIC OTP / EXPIRATION WORKER
# =========================================================
...
def _order_expired(order, runtime_provider=None):
    runtime_provider = runtime_provider or {}
    value = order.get("expired_at") or runtime_provider.get("expired_at")
    dt = _parse_provider_time(value)
...
async def _send_traffic_otp_notification(application, order, code, sms_text):
    ...
    if not TRAFFIC_CHANNEL:
        logger.error("[TRAFFIC] TRAFFIC_CHANNEL belum diatur; OTP notification tidak dapat dikirim")
        return False
    try:
...
async def _send_otp_received_message(application, order_id, data_sms):
    ...
    # from making the OTP effectively disappear from the bot.
    try:
        changed = await asyncio.to_thread(mark_order_success, order_id)
    except Exception:
        logger.exception("[AUTO OTP] failed to mark SUCCESS order=%s", order_id)
        return False
    if not changed:
        return False
...
def _get_cancel_order_lock(order_id):
    """Return one asyncio lock per local order so manual/auto cancel cannot race."""
    lock = _CANCEL_ORDER_LOCKS.get(order_id)
    if lock is None:
        lock = asyncio.Lock()
        _CANCEL_ORDER_LOCKS[order_id] = lock
    return lock


def _provider_status_values(value):
    """Collect status/state fields from common provider response envelopes."""
    found = []
    if isinstance(value, dict):
        ...
        for key in ("data", "result", "order", "activation"):
            ...
            if isinstance(nested, dict):
                found.extend(_provider_status_values(nested))
    return found


CANCELLED_PROVIDER_STATES = {
    "cancel", "canceled", "cancelled", "canceled_by_user", "cancelled_by_user"
}


async def _cancel_provider_and_verify(provider, provider_order_id, order_id=""):
    """Cancel upstream order and require explicit provider cancellation confirmation."""
    provider = str(provider or "5sim").strip().lower()
    ...
    try:
        ...
        try:
            ...
            return {"response": "ERROR", "error": f"Verifikasi 5SIM gagal: {exc}", "raw": result}
        verify_statuses = _provider_status_values(verify)
        if any(status in CANCELLED_PROVIDER_STATES for status in verify_statuses):
            return {"response": "OK", "provider_status": verify_statuses, "raw": verify}
        return {
            "response": "ERROR",
            "error": "Pembatalan 5SIM belum terkonfirmasi oleh provider.",
            "provider_status": verify_statuses,
            "raw": result,
        }
    except Exception as exc:
...
async def _auto_expire_order(application, order, provider_order_id):
    """Cancel an expired order safely, then refund only after provider confirmation."""
    ...
    async with lock:
        ...
        try:
            refund = await asyncio.to_thread(
                refund_order,
                order_id,
                "Order otomatis dibatalkan karena waktu expired habis tanpa OTP diterima.",
            )
        except Exception:
    ...
    try:
        await application.bot.send_message(
            ...
            text=(
                "⏰ <b>PESANAN OTOMATIS DIBATALKAN</b>\n\n"
                f"🧾 Order: <code>{escape(order_id)}</code>\n"
                "Waktu aktif nomor sudah habis dan OTP belum diterima.\n\n"
                f"💸 Refund: <b>{format_rupiah(order.get('sell_price') or 0)}</b>\n"
                f"💰 Saldo sekarang: <b>{format_rupiah(refund.get('balance') or 0)}</b>"
            ),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
            ...
            if rows:
                ...
                if total and selected is None:
                    for offset in range(total):
                        ...
                        candidate = rows[idx]
                        provider = str(candidate.get("provider") or "5sim").strip().lower()
                        if provider not in _AUTO_POLL_NEXT:
                            provider = "5sim"
                        if now_mono >= _AUTO_POLL_NEXT[provider]:
                            selected = candidate
...
async def process_otp_order(
    ...
    await query.edit_message_text(
        ...
        f"📞 Nomor:\n<code>{phone}</code>\n\n"
        f"💰 Harga: <b>{format_rupiah(sell_price)}</b>\n"
        f"🕐 Waktu transaksi: <b>{format_datetime_wib((get_order(order_id) or {}).get('created_at'))}</b>\n"
        f"⏰ Waktu expired: <b>{format_datetime_wib(provider_expired_at)}</b>\n"
...
Jika OTP tidak masuk, tekan <b>❌ Batal / Refund</b>."""
    ...
    if data.startswith(
        ...
        if order_status in {"SUCCESS", "COMPLETED"} and order.get("otp_code"):
            service_name = order.get("service_name") or order.get("service") or "-"
            country_name = order.get("country_name") or order.get("country") or "-"
            phone = order.get("phone") or "-"
            expired_at = order.get("expired_at") or "-"
            text_body = (
                "🎉 <b>OTP DITERIMA</b>\n\n"
                f"🧾 Order: <code>{escape(str(order_id))}</code>\n"
                f"📱 Layanan: <b>{escape(str(service_name))}</b>\n"
                ...
                f"⏰ Expired: <b>{escape(format_datetime_wib(expired_at))}</b>\n\n"
                f"🔐 OTP: <code>{escape(str(order.get('otp_code')))}</code>\n\n"
            ...
            if order_status == "SUCCESS":
                ...
                buttons.append([InlineKeyboardButton("✅ Pesanan Selesai", callback_data=f"otp_finish:{order_id}")])
            else:
                buttons.append([InlineKeyboardButton("🏠 Menu Utama", callback_data="user_home")])
            await query.edit_message_text(text_body, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buttons))
            return

        if order_status != "PENDING":
            await query.answer("OTP belum tersedia di order ini.", show_alert=True)
            return
        ...
        if sms_list:
            ...
            if _is_real_otp_code(code):
                data_sms["sms"] = [{"code": str(code), "text": str(text)}]
                ...
                if processed:
                    try:
                        await query.answer("✅ OTP berhasil dikirim.", show_alert=False)
                    except Exception:
                        pass
                else:
                    await query.answer(
                        "⚠️ OTP ditemukan, tetapi pengiriman Telegram belum berhasil. Bot akan mencoba lagi.",
                        show_alert=True,
                    )
                return
    ...
    if data.startswith("otp_resend:"):
        ...
        if provider == "5sim":
            ...
            await query.answer(
                str(
                    (result or {}).get("error")
                    or (
                        "5SIM tidak menyediakan request resend OTP "
                        "untuk order activation yang sama."
                    )
                ),
                show_alert=True
            )
        ...
        await query.answer(
            "Provider order ini tidak mendukung resend OTP.",
            show_alert=True
        )
        return
    ...
    if data.startswith("otp_finish:"):
        ...
        if not order or int(order["telegram_id"]) != int(user_id):
            ...
            return
        if str(order.get("status") or "").upper() != "SUCCESS":
            await query.answer("Pesanan belum siap diselesaikan.", show_alert=True)
            return
        ...
        await query.edit_message_text(
            "✅ <b>PESANAN SELESAI</b>\n\n"
            f"🧾 Order: <code>{escape(order_id)}</code>\n"
            "Pesanan sudah ditutup dan tidak bisa digunakan lagi.\n\n"
    ...
    if data.startswith("otp_cancel:"):
        ...
        # Answer the callback immediately. This prevents one user's 120-second
        # process from occupying Telegram's update handler and blocking other
        # users from pressing Batal / Refund.
        state = _CANCEL_COUNTDOWN_STATE.get(order_id)
        task = _CANCEL_TASKS.get(order_id)
        loop = asyncio.get_running_loop()

        if task and not task.done() and state:
            deadline = float(state.get("deadline", loop.time()))
            ...
            if phase == "refunded":
            ...
            elif phase == "refund_failed":
                await query.answer(
                    "⚠️ Provider sudah dibatalkan, tetapi pengembalian saldo belum selesai. Silakan hubungi admin.",
                    show_alert=True,
                )
            elif phase == "error":
                await query.answer(
                    "⚠️ Proses Batal / Refund mengalami kendala. Silakan coba lagi.",
                    show_alert=True,
                )
                ...
                await query.answer(
                    ...
                    "Waktu tetap berjalan meskipun kamu menekan OK atau membuka popup lagi.",
                    show_alert=True,
                )
            else:
                await query.answer(
        ...
        # Do lightweight ownership/status checks off the event loop. The popup
        # itself is still returned as quickly as possible after the checks.
        ...
        # is created in the background, so other users/orders remain responsive.
        ...
        async def _run_user_cancel():
            try:
                # Wait until the exact 120-second deadline. No provider request
                # is made before this point.
                while True:
                    current = await asyncio.to_thread(get_order, order_id)
                    if (
                        not current
                        or str(current.get("status") or "").upper() != "PENDING"
                        or _order_has_received_otp(current)
                    ):
                        return
                    remaining_float = deadline - asyncio.get_running_loop().time()
                    if remaining_float <= 0:
                        break
                    _CANCEL_COUNTDOWN_STATE[order_id]["remaining"] = max(
                        1, int(remaining_float + 0.999)
                    )
                    await asyncio.sleep(min(1.0, remaining_float))

                # Re-read immediately at the deadline. This avoids cancelling
                # a number that received OTP or was auto-refunded at the same time.
                current = await asyncio.to_thread(get_order, order_id)
                if (
                    not current
                    or str(current.get("status") or "").upper() != "PENDING"
                ...
                async with lock:
                    ...
                    provider = str(current.get("provider") or "5sim").strip().lower()
                    cancel_result = await _cancel_provider_and_verify(
                        provider, provider_order_id, order_id
                    )
                    if not cancel_result or cancel_result.get("response") != "OK":
                        logger.warning(
                            "[OTP CANCEL] provider cancel not confirmed order=%s result=%s",
                            order_id, cancel_result,
                        )
                        _CANCEL_COUNTDOWN_STATE[order_id]["phase"] = "provider_failed"
                        return
                    ...
                    try:
                        ...
                        logger.exception("[OTP CANCEL] refund failed order=%s", order_id)
                        _CANCEL_COUNTDOWN_STATE[order_id]["phase"] = "refund_failed"
                        return
                    ...
                    try:
                        message_id = (
                            ...
                            or (_RUNTIME_PROVIDER_CACHE.get(order_id) or {}).get("telegram_message_id")
                            or (query.message.message_id if query.message else None)
                        )
                        if message_id:
                ...
                logger.exception("[OTP CANCEL] background cancellation failed order=%s", order_id)
                state = _CANCEL_COUNTDOWN_STATE.get(order_id)
                if state:
                    state["phase"] = "error"
            finally:
                _ACTIVE_CANCEL_COUNTDOWNS.discard(order_id)
                # Keep the task/state alive briefly so a user who taps the
                # button immediately after completion gets a final popup
                # instead of accidentally starting another countdown.
                await asyncio.sleep(5)
                _CANCEL_TASKS.pop(order_id, None)
                _CANCEL_COUNTDOWN_STATE.pop(order_id, None)

        _CANCEL_TASKS[order_id] = asyncio.create_task(_run_user_cancel())
        return
    ...
    if data.startswith("cancel_manual:"):
        ...
        with get_db() as db:
            db.execute(
                "UPDATE deposits SET status='FAILED', confirmed_at=%s WHERE deposit_id=%s AND telegram_id=%s AND payment_method='MANUAL_QRIS' AND status='PENDING'",
                (now(), deposit_id, user_id)
            )
        context.chat_data["waiting_deposit"] = False
        context.chat_data.pop("pending_deposit_amount", None)
    ...
    if data == "cek_deposit":
        ...
        if not status_data:
            ...
            await query.edit_message_text(
                ...
                "❌ Gagal mengecek pembayaran otomatis.",
...
async def _admin_cancel_order(query, context, order_id):
    ...
    try:
        ...
        # Refund is complete: update the user's original ORDER BERHASIL card
        # to ORDER DIBATALKAN / REFUND. Do not create a duplicate order card.
        try:
            user_message_id = (
            ...
            if user_message_id:
                refund_amount = result.get("amount") or order.get("sell_price") or 0
                balance_after = result.get("balance")
                balance_text = format_rupiah(balance_after) if balance_after is not None else "-"
                await context.bot.edit_message_text(
                    chat_id=int(order["telegram_id"]),
                    ...
                    text=(
                        ...
                        f"💸 Refund: <b>{format_rupiah(refund_amount)}</b>\n"
                        f"💳 Saldo sekarang: <b>{balance_text}</b>\n\n"
                        "Pesanan telah dibatalkan oleh admin dan saldo sudah dikembalikan."
                    ),
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("📱 ORDER LAGI", callback_data="order")],
            ...
            logger.exception("[ADMIN CANCEL] gagal update kartu order user=%s", order_id)
...
async def _admin_deposits_page(query, status="ALL", page=0):
    ...
    with get_db() as db:
        if status == "ALL":
            ...
            rows = db.execute(
                """SELECT deposit_id, telegram_id, amount, status, payment_method, payment_amount, created_at
    ...
    for row in rows:
        keyboard.append([InlineKeyboardButton(
            f"{format_rupiah(row['amount'])} • {('QRIS' if row.get('payment_method') == 'MANUAL_QRIS' else 'AUTO')} • {row['status']} • ID {row['telegram_id']}",
...
async def admin_callback(
    ...
    if query.data == "admin_users":
        ...
        current = is_maintenance_enabled()
        set_bot_setting("maintenance", "0" if current else "1")
        state = "AKTIF" if not current else "NONAKTIF"
        await query.edit_message_text(
            "🛠 <b>MAINTENANCE BOT</b>\n\n"
            f"Status maintenance sekarang: <b>{state}</b>.\n\n"
            "Saat aktif, user tidak dapat membuat order OTP baru.\n"
            "Admin tetap dapat membuka dan menggunakan seluruh menu admin.",
            parse_mode="HTML",
            reply_markup=admin_menu(),
        )
...
async def button_handler(
    ...
    if is_maintenance_enabled() and not is_admin(user_id):
        ...
        if query.data == "order" or query.data.startswith(blocked_prefixes):
            await query.answer("🛠 Maintenance aktif. Pemesanan OTP sementara ditutup.", show_alert=True)
            return
...
async def photo_handler(update, context):
    if not update.message or not update.message.photo:
        return
    if not is_admin(update.effective_user.id) or not context.user_data.get("waiting_qris_setup", False):
        return
    file_id = update.message.photo[-1].file_id
    set_bot_setting("qris_manual_file_id", file_id)
    context.user_data["waiting_qris_setup"] = False
    await update.message.reply_text("✅ <b>QRIS manual berhasil disimpan.</b>\n\nQRIS ini akan dikirim ke user saat memilih <b>QRIS Manual</b>.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Admin Panel", callback_data="admin_home")]]))
...
async def text_handler(
    ...
    if is_admin(update.effective_user.id):
        ...
        if uid in ADMIN_SEARCH_DEPOSITS:
            ...
            with get_db() as db:
                rows = db.execute(
                    """SELECT deposit_id, amount, status, payment_method, payment_amount, created_at FROM deposits WHERE telegram_id = %s ORDER BY created_at DESC LIMIT 30""",
                    (target_id,),
                ).fetchall()
